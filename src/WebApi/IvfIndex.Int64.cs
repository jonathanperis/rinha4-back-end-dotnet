...
internal sealed partial class IvfIndex
{
    ...
    private byte FraudCountOnceLong(
        ...
        int candidateCapacity = 5)
    {
        if (candidateCapacity == 5 && useFloatAvx2 && Avx2.IsSupported && blockLanes == 8)
        {
        ...
        Span<long> candidateDistances = candidateCapacity <= 256 ? stackalloc long[candidateCapacity] : new long[candidateCapacity];
        Span<int> candidateIds = candidateCapacity <= 256 ? stackalloc int[candidateCapacity] : new int[candidateCapacity];
        Span<byte> candidateLabels = candidateCapacity <= 256 ? stackalloc byte[candidateCapacity] : new byte[candidateCapacity];
