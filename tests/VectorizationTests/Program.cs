...
VectorizationTestRunner.Run("direct parse quantization matches legacy request quantization", () =>
{
    ...
    FraudScorer.QuantizeRequestForTest(input, expected, 1024, 10000.0, 12, 10.0, 525600, 20000, 500, 10000, riskByCode, knownByCode);
    bool ok = FraudScorer.TryParseAndQuantizeForTest(payload, actual, 1024, 10000.0, 12, 10.0, 525600, 20000, 500, 10000, riskByCode, knownByCode);

    if (!ok)
        throw new InvalidOperationException("expected direct parser quantization to succeed");
    for (int i = 0; i < 16; i++)
...
VectorizationTestRunner.Run("reads one-pass full repair as IVF default", () =>
{
    using var env = new ScopedEnvironment(
        ...
        ("IVF_PEDRO_DECISION_PATH", null),
        ("IVF_BORDERLINE_MIN_FRAUDS", null),
        ("IVF_BORDERLINE_MAX_FRAUDS", null),
        ("IVF_BORDERLINE_NPROBE", null),
        ("IVF_BORDERLINE_RERANK", null));
    ...
    VectorizationTestRunner.AssertEqualInt(0, options.PedroDecisionPath ? 1 : 0);
    VectorizationTestRunner.AssertEqualInt(2, options.BorderlineMinFrauds);
    VectorizationTestRunner.AssertEqualInt(3, options.BorderlineMaxFrauds);
    VectorizationTestRunner.AssertEqualInt(32, options.BorderlineNProbe);
    VectorizationTestRunner.AssertEqualInt(128, options.BorderlineRerank);
});

VectorizationTestRunner.Run("reads Pedro-style IVF decision path controls", () =>
{
    using var env = new ScopedEnvironment(
        ("IVF_PEDRO_DECISION_PATH", "1"),
        ("IVF_BORDERLINE_MIN_FRAUDS", "1"),
        ("IVF_BORDERLINE_MAX_FRAUDS", "4"),
        ("IVF_BORDERLINE_NPROBE", "16"),
        ("IVF_BORDERLINE_RERANK", "64"));

    IvfSearchOptions options = IvfSearchOptions.FromEnvironment();

    VectorizationTestRunner.AssertEqualInt(1, options.PedroDecisionPath ? 1 : 0);
    VectorizationTestRunner.AssertEqualInt(1, options.BorderlineMinFrauds);
    VectorizationTestRunner.AssertEqualInt(4, options.BorderlineMaxFrauds);
    VectorizationTestRunner.AssertEqualInt(16, options.BorderlineNProbe);
    VectorizationTestRunner.AssertEqualInt(64, options.BorderlineRerank);
});

VectorizationTestRunner.Run("Pedro-style IVF decision path expands borderline counts", () =>
{
    string path = Path.Combine(Path.GetTempPath(), $"rinha-ivf-pedro-test-{Guid.NewGuid():N}.bin");
    try
    {
        IvfTestIndex.Write(path, 2, 5);
        if (!IvfIndex.TryLoad(path, out IvfIndex? index, out string error) || index is null)
            throw new InvalidOperationException(error);

        Span<short> quantized = stackalloc short[16];
        byte fastOnly = index.FraudCount(quantized, new IvfSearchOptions(1, 1, false, false, 1, 4, 5_000_000, 4_500_000, false, 0, false, 2, 3, 2, 128));
        byte expanded = index.FraudCount(quantized, new IvfSearchOptions(1, 1, false, false, 1, 4, 5_000_000, 4_500_000, false, 0, true, 2, 3, 2, 128));

        VectorizationTestRunner.AssertEqualInt(2, fastOnly);
        VectorizationTestRunner.AssertEqualInt(5, expanded);
    }
    finally
    {
        if (File.Exists(path))
...
VectorizationTestRunner.Run("Pedro-style IVF decision path leaves non-borderline counts fast", () =>
{
    string path = Path.Combine(Path.GetTempPath(), $"rinha-ivf-pedro-fast-test-{Guid.NewGuid():N}.bin");
    try
    {
        IvfTestIndex.Write(path, 0, 5);
        if (!IvfIndex.TryLoad(path, out IvfIndex? index, out string error) || index is null)
            throw new InvalidOperationException(error);

        Span<short> quantized = stackalloc short[16];
        byte result = index.FraudCount(quantized, new IvfSearchOptions(1, 1, false, false, 1, 4, 5_000_000, 4_500_000, false, 0, true, 2, 3, 2, 128));

        VectorizationTestRunner.AssertEqualInt(0, result);
    }
    finally
    {
        if (File.Exists(path))
...
internal static class IvfRepairAssertions
{
    ...
    public static void AssertBoundaryRepair(string path, byte firstPassFrauds, byte repairFrauds, byte expectedFastOnly, byte expectedRepaired)
    {
        ...
        byte fastOnly = index.FraudCount(quantized, new IvfSearchOptions(1, 1, false, false, 1, 4, 5_000_000, 4_500_000, false, 0, false, 2, 3, 32, 128));
        byte repaired = index.FraudCount(quantized, new IvfSearchOptions(1, 1, true, true, 1, 4, 5_000_000, 4_500_000, false, 0, false, 2, 3, 32, 128));
